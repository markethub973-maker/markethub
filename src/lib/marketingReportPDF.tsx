...
function PageFooter({ client, period }: { client: string; period: string }) {
  return (
    <View style={styles.footer}>
      <Text style={styles.footerText}>MarketHub Pro — Raport Marketing</Text>
    ...
    <Document>
      ...
      <Page size="A4" style={styles.coverPage}>
        <View>
          ...
          <Text style={[styles.h1, { fontSize: 32, marginBottom: 8 }]}>Raport Marketing</Text>
          ...
          <View style={{ flexDirection: "row", gap: 20, marginBottom: 40 }}>
            ...
            {facebook && (
              <View style={{ flex: 1, backgroundColor: "rgba(24,119,242,0.08)", borderRadius: 10, padding: 16, borderWidth: 1, borderColor: "rgba(24,119,242,0.2)" }}>
                ...
                <Text style={{ fontSize: 11, color: "#C4AA8A" }}>{fmtNum(facebook.followers)} urmăritori</Text>
              </View>
            )}
          </View>
          ...
          <View style={{ flexDirection: "row", gap: 12 }}>
            {[
              { label: "Reach 30 zile", value: fmtNum(reach30), color: pink },
        ...
        <View>
          <Text style={{ fontSize: 8, color: "#7B6B5A" }}>Generat pe {generatedAt} · markethubpromo.com</Text>
        </View>
      </Page>

      {/* ═══ PAGE 2: KPIs + BEST DAYS + CONTENT MIX ═══ */}
      <Page size="A4" style={styles.page}>
        <Text style={[styles.h2, { marginBottom: 16 }]}>Performanță Generală</Text>

        {/* KPI Row */}
        <View style={styles.row}>
          <KpiCard label="Followers Instagram" value={fmtNum(instagram.followers)} sub={`${instagram.mediaCount} postări totale`} color={pink} />
          <KpiCard label="Reach 30 zile" value={fmtNum(reach30)} sub="persoane unice" color={amber} />
          <KpiCard label="Impressions 30 zile" value={fmtNum(impressions30)} sub="afișări totale" color={purple} />
          <KpiCard label="Eng. Rate Mediu" value={avgEngRate.toFixed(2) + "%"} sub="per postare" color={avgEngRate >= 3 ? green : amber} />
        </View>
        ...
        <View style={styles.row}>
          ...
          <View style={[styles.card, { flex: 1, marginBottom: 0 }]}>
            <Text style={styles.h3}>Cele mai bune zile de postare</Text>
          ...
          {/* Content Mix */}
          <View style={[styles.card, { flex: 1, marginBottom: 0 }]}>
            <Text style={styles.h3}>Mix Conținut</Text>
            {contentMix.map((c, i) => {
            ...
            <Text style={[styles.small, { marginTop: 8 }]}>
              {contentMix.find(c => c.name === "Video") ? "Video-urile generează reach mai mare." : "Adaugă mai mult video/reels pentru reach crescut."}
            </Text>
          </View>
        </View>
      ...
      {/* ═══ PAGE 3: TOP POSTS ═══ */}
      <Page size="A4" style={styles.page}>
        <Text style={[styles.h2, { marginBottom: 4 }]}>Top Postări</Text>
        <Text style={[styles.text, { marginBottom: 16 }]}>Cele mai performante postări din ultimele 30 de zile, sortate după engagement.</Text>
            ...
            <View key={i} style={styles.postCard}>
              <View style={{ backgroundColor: "rgba(225,48,108,0.06)", padding: 10 }}>
                <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                  ...
                  <Text style={{ fontSize: 8, color: light }}>{p.mediaType === "VIDEO" ? "Video" : p.mediaType === "CAROUSEL_ALBUM" ? "Carusel" : "Foto"}</Text>
                </View>
                {p.caption && (
        ...
        {/* Summary table */}
        <Text style={[styles.h3, { marginBottom: 8 }]}>Tabel Sumar Postări</Text>
        <View style={{ borderWidth: 1, borderColor: border, borderRadius: 6, overflow: "hidden" }}>
          <View style={{ flexDirection: "row", backgroundColor: "rgba(245,215,160,0.12)", padding: 8 }}>
            {["#", "Tip", "Likes", "Comentarii", "ER%", "Data"].map(h => (
          ...
          {topPosts.slice(0, 8).map((p, i) => (
            <View key={i} style={{ flexDirection: "row", padding: 7, borderTopWidth: 1, borderColor: border }}>
              ...
              <Text style={{ fontSize: 8, color: muted, flex: 1 }}>{p.mediaType === "VIDEO" ? "Video" : p.mediaType === "CAROUSEL_ALBUM" ? "Carusel" : "Foto"}</Text>
