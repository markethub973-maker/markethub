      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 sticky top-0 z-10" style={{ backgroundColor: "var(--color-bg-secondary)", borderBottom: "1px solid rgba(245,215,160,0.2)" }}>
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-lg" style={{ color: "var(--color-text)" }}>Comparatie Canale</h2>
          </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[a, b].map((ch, i) => (
              <div key={ch.id} className="rounded-xl p-4 flex items-center gap-3" style={{ backgroundColor: i === 0 ? "rgba(245,158,11,0.08)" : "rgba(120,97,78,0.08)", border: `1px solid ${i === 0 ? "rgba(245,158,11,0.25)" : "rgba(120,97,78,0.2)"}` }}>
                ...
                <div>
                  <p className="font-bold text-sm" style={{ color: "var(--color-text)" }}>{ch.name}</p>
          ...
          {/* Chart */}
          <div>
            <h3 className="font-semibold mb-3" style={{ color: "var(--color-text)" }}>Comparatie vizuala</h3>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={chartData} margin={{ left: -10, right: 8 }}>
                ...
                <Bar dataKey={a.name} fill="var(--color-primary)" radius={[4, 4, 0, 0]} />
          ...
          <div className="rounded-xl p-4" style={{ background: "linear-gradient(135deg, #F59E0B22, #D9770622)", border: "1px solid rgba(245,158,11,0.25)" }}>
            <p className="text-sm font-semibold mb-1" style={{ color: "var(--color-text)" }}>Verdict</p>
