...
export default function ExportButtons({ filename, sheets, compact = false, className = "" }: ExportButtonsProps) {
    ...
    return (
      <div className={`flex items-center gap-1 ${className}`}>
        <button
          ...
          title="Export CSV"
          className="p-1.5 rounded text-[#78614E] hover:bg-[#F5D7A0]/30 transition-colors"
        >
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <button
        onClick={handleCsvExport}
        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border border-[#E8D9C5] bg-white text-[#78614E] hover:bg-[#FFF8F0] transition-colors"
      >
      ...
      <button
        onClick={handleExcelExport}
        disabled={loadingExcel}
        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-[#16a34a] text-white hover:bg-[#15803d] transition-colors disabled:opacity-60"
      >
        {loadingExcel ? (
