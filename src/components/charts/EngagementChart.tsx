  ...
  return (
    <div className="rounded-xl p-5" style={{ backgroundColor: "var(--color-bg-secondary)", border: "1px solid rgba(245,215,160,0.25)", boxShadow: "0 1px 3px rgba(120,97,78,0.08)" }}>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="font-semibold" style={{ color: "var(--color-text)" }}>Engagement Rate — Trending RO</h3>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={data} layout="vertical" margin={{ top: 4, right: 20, bottom: 0, left: 8 }}>
            ...
            <Bar dataKey="er" radius={[0, 4, 4, 0]}>
              {data.map((d, i) => (
                <Cell key={i} fill={d.er > 1 ? "var(--color-primary)" : d.er > 0.5 ? "var(--color-primary-hover)" : "rgba(245,158,11,0.4)"} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
