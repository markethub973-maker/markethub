...
export default function InstagramSearchPage() {
  ...
  const searchUser = async (q?: string) => {
    const term = (q || username).trim().replace(/^@/, "");
    if (!term) return;
  ...
  return (
    <div>
      <Header title="Instagram Search" subtitle="Analyze any public Instagram profile - followers, engagement, recent posts" />
      <div className="p-6 space-y-5">
        ...
        <div className="rounded-xl p-5" style={cardStyle}>
          <div className="flex gap-3 flex-wrap">
            ...
            <button
              ...
              style={{
                background: `linear-gradient(135deg, ${IG3}, ${IG}, ${IG2})`,
                color: "white",
            ...
            <div className="rounded-xl overflow-hidden" style={cardStyle}>
                ...
                <div className="absolute -bottom-10 left-6">
                  <div className="w-20 h-20 rounded-full border-4 overflow-hidden" style={{ borderColor: "var(--color-bg-secondary)" }}>
                    {result.profile.avatar ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center" style={{ backgroundColor: `${IG}20` }}>
                        <Users className="w-8 h-8" style={{ color: IG }} />
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="pt-12 px-6 pb-5">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {result.profile.isPrivate && (
                        <Lock className="w-4 h-4" style={{ color: "#A8967E" }} />
                      )}
                    </div>
                    <a
                      href={`https://instagram.com/${result.profile.username}`}
                      target="_blank"
                    ...
                    {result.profile.category && (
                      <p className="text-xs mt-1" style={{ color: "#A8967E" }}>{result.profile.category}</p>
                    )}
                  </div>
                ...
                {result.profile.biography && (
                  <p className="text-sm mt-3 whitespace-pre-line" style={{ color: "#78614E" }}>
                    {result.profile.biography}
                  </p>
                )}
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                  {result.posts.map(post => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="rounded-xl overflow-hidden group transition-all hover:shadow-lg"
                      style={{ ...cardStyle, border: `1px solid ${IG}15` }}
                    >
        ...
        {/* Popular accounts (before first search) */}
        {!searched && (
          <div className="rounded-xl p-5" style={cardStyle}>
            <div className="flex items-center gap-2 mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {POPULAR_ACCOUNTS.map(acc => (
                <button
                  key={acc}
                  type="button"
                  onClick={() => { setUsername(acc); searchUser(acc); }}
                  ...
                  onMouseEnter={e => {
                    e.currentTarget.style.background = `linear-gradient(135deg, ${IG}25, ${IG2}25)`;
                    e.currentTarget.style.borderColor = IG;
                  }}
                  onMouseLeave={e => {
                    e.currentTarget.style.background = `linear-gradient(135deg, ${IG}10, ${IG2}10)`;
                    e.currentTarget.style.borderColor = `${IG}20`;
                  }}
                >
                  @{acc}
                </button>
