...
export async function POST(req: NextRequest) {
  ...
  const input: Record<string, unknown> = { resultsLimit: Math.min(limit, 30) };
  ...
  const result = await safeApify<any[]>("apify~instagram-scraper", input, { timeoutSec: 90, retries: 1 });
