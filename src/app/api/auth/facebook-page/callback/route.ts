...
export async function GET(req: NextRequest) {
  ...
  if (!pagesRes.ok) return NextResponse.redirect(new URL("/settings?error=pages_fetch_failed", req.url));
  const { data: pages } = await pagesRes.json();

  if (!pages?.length) return NextResponse.redirect(new URL("/settings?error=no_pages_found", req.url));
  ...
  return NextResponse.redirect(new URL("/settings?fb_page=connected", req.url));
}
