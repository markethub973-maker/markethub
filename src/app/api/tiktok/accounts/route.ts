...
export async function POST(req: NextRequest) {
  ...
  if (action === "set_primary") {
    await svc.from("tiktok_connections" as any).update({ is_primary: false }).eq("user_id", auth.userId);
    await svc.from("tiktok_connections" as any).update({ is_primary: true }).eq("user_id", auth.userId).eq("tiktok_open_id", tiktok_open_id);
    return NextResponse.json({ ok: true });
  }

  if (action === "disconnect") {
    await svc.from("tiktok_connections" as any).delete().eq("user_id", auth.userId).eq("tiktok_open_id", tiktok_open_id);
    ...
    const { data: remaining } = await svc
      ...
      .limit(1);
    if (remaining?.[0]) {
      await svc.from("tiktok_connections" as any).update({ is_primary: true }).eq("user_id", auth.userId).eq("tiktok_open_id", (remaining[0] as any).tiktok_open_id);
    }
    return NextResponse.json({ ok: true });
  }
