...
import { BUSINESS_BRIEF_SYSTEM_PROMPT, buildPlaylistStrategyPrompt } from "@/lib/ai-prompts";
...
export async function POST(req: NextRequest) {
  ...
  if (remaining < AI_ACTION_COSTS.playlist_strategy) {
  ...
  if (!playlists?.length) return NextResponse.json({ error: "Playlists array required" }, { status: 400 });
  ...
  try {
    ...
    const message = await client.messages.create({
      ...
      messages: [{ role: "user", content: buildPlaylistStrategyPrompt(playlists) }],
    });
    ...
    await supabase.from("usage_tracking").insert({
      user_id: auth.userId, feature: "playlist_strategy", api_name: "anthropic",
    ...
    console.error("[PlaylistStrategy API] Error:", e?.status, e?.error?.type, e?.message);
