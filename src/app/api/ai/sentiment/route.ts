...
import { BUSINESS_BRIEF_SYSTEM_PROMPT, buildSentimentPrompt } from "@/lib/ai-prompts";
...
export async function POST(req: NextRequest) {
  ...
  if (remaining < AI_ACTION_COSTS.sentiment_analysis) {
  ...
  const { comments, platform, contentTitle } = await req.json();
  ...
  try {
    ...
    await supabase.from("usage_tracking").insert({
      user_id: auth.userId, feature: "sentiment_analysis", api_name: "anthropic",
    ...
    console.error("[Sentiment API] Error:", e?.status, e?.error?.type, e?.message);
