...
import { BUSINESS_BRIEF_SYSTEM_PROMPT, buildCommentFAQPrompt } from "@/lib/ai-prompts";
...
export async function POST(req: NextRequest) {
  ...
  if (remaining < AI_ACTION_COSTS.comment_faq) {
  ...
  const { comments, channelNiche, videoTitle } = await req.json();
  if (!comments?.length) return NextResponse.json({ error: "Comments array required" }, { status: 400 });
  ...
  try {
    ...
    const message = await client.messages.create({
      ...
      messages: [{ role: "user", content: buildCommentFAQPrompt(comments, channelNiche, videoTitle) }],
    });
    ...
    console.error("[CommentFAQ API] Error:", e?.status, e?.error?.type, e?.message);
