...
export async function POST(req: NextRequest) {
  ...
  if (remaining < AI_ACTION_COSTS.category_analysis) {
  ...
  const { categories, niche } = await req.json();
  if (!categories?.length) return NextResponse.json({ error: "Categories array required" }, { status: 400 });
  ...
  try {
    ...
    const message = await client.messages.create({
      ...
      messages: [{ role: "user", content: buildCategoryAnalysisPrompt(categories, niche ?? "general") }],
    });
    ...
    console.error("[CategoryAnalysis API] Error:", e?.status, e?.error?.type, e?.message);
