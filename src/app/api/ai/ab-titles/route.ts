...
import { BUSINESS_BRIEF_SYSTEM_PROMPT, buildABTitlesPrompt } from "@/lib/ai-prompts";
...
export async function POST(req: NextRequest) {
  ...
  if (remaining < AI_ACTION_COSTS.ab_titles) {
  ...
  const { title, description, niche, platform } = await req.json();
  ...
  try {
    ...
    const message = await client.messages.create({
      ...
      messages: [{ role: "user", content: buildABTitlesPrompt(title, description ?? "", niche ?? "general", platform ?? "YouTube") }],
    });
    ...
    await supabase.from("usage_tracking").insert({
      user_id: auth.userId, feature: "ab_titles", api_name: "anthropic",
    ...
    console.error("[AB Titles API] Error:", e?.status, e?.error?.type, e?.message);
