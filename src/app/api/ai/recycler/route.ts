...
const SYSTEM_PROMPT = `Take this old social media post and transform it into 3 fresh new versions. Keep the core message but change the angle, hook, and CTA. Each version should feel completely new.

Output STRICT JSON:
{
  "versions": [
    "Version 1 — completely new angle and hook",
    "Version 2 — different framing and CTA",
    "Version 3 — fresh perspective, new opening"
  ]
}

Rules:
- Language must match the input language.
- Each version must be standalone (no references to the original).
- Keep the core message/lesson but change: opening hook, supporting argument, CTA.
- Each version should target a different emotional angle (curiosity, urgency, empathy).
- Preserve hashtag usage if the original had them.`;
...
export async function POST(req: NextRequest) {
  ...
  if (!input?.trim()) return NextResponse.json({ error: "Original post content is required" }, { status: 400 });
  ...
  try {
    ...
    const result = JSON.parse(jsonMatch[0]) as { versions?: string[] };

    const inputTokens = message.usage?.input_tokens ?? 500;
    const outputTokens = message.usage?.output_tokens ?? 1000;
    const actualCost = (inputTokens * 0.8 + outputTokens * 4) / 1_000_000;
    await supabase.from("usage_tracking").insert({
      user_id: auth.userId, feature: "recycler", api_name: "anthropic",
      cost_usd: parseFloat(actualCost.toFixed(6)), month_year: currentMonth,
      timestamp: new Date().toISOString(),
    });

    return NextResponse.json({
      versions: (result.versions ?? []).slice(0, 3),
      ai_budget: {
    ...
    console.error("[Recycler API] Error:", e?.status, e?.error?.type, e?.message);
