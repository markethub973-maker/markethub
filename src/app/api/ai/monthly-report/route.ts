...
import { BUSINESS_BRIEF_SYSTEM_PROMPT, buildMonthlyReportPrompt } from "@/lib/ai-prompts";
...
export async function POST(req: NextRequest) {
  ...
  if (remaining < AI_ACTION_COSTS.monthly_report) {
  ...
  if (!metrics?.platform) return NextResponse.json({ error: "Platform is required" }, { status: 400 });
  ...
  try {
    ...
    const message = await client.messages.create({
      ...
      messages: [{ role: "user", content: buildMonthlyReportPrompt(metrics) }],
    });
    ...
    await supabase.from("usage_tracking").insert({
      user_id: auth.userId, feature: "monthly_report", api_name: "anthropic",
    ...
    console.error("[Monthly Report API] Error:", e?.status, e?.error?.type, e?.message);
