...
function StatusIcon({ value }: { value: "yes" | "no" | "partial" }) {
  ...
  return                          <Minus className="w-4 h-4" style={{ color: "#F59E0B" }} />;
}
      ...
      <main className="max-w-5xl mx-auto px-6 py-10 space-y-12">
          ...
          <ul className="space-y-3">
            {cmp.why_switch_bullets.map((b, i) => (
              ...
              >
                ...
                <p className="text-sm" style={{ color: "#292524", lineHeight: 1.5 }}>{b}</p>
              </li>
            ))}
          </ul>
        </section>
        ...
        <section>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ backgroundColor: "rgba(0,0,0,0.03)" }}>
                  <th className="text-left px-4 py-3 font-bold" style={{ color: "#292524" }}>Feature</th>
                  <th className="text-center px-4 py-3 font-bold" style={{ color: "#D97706" }}>MarketHub</th>
              ...
              <tbody>
                {cmp.feature_table.map((row, i) => (
                  ...
                  >
                    <td className="px-4 py-3" style={{ color: "#292524" }}>
                      <div>{row.feature}</div>
                      {row.note && (
          ...
          <p className="text-[11px] mt-2 flex items-center gap-3 flex-wrap" style={{ color: "#A8967E" }}>
            ...
            <span><Minus className="w-3 h-3 inline" style={{ color: "#F59E0B" }} /> partial / paid add-on</span>
        ...
        <section className="text-center">
          <p className="text-xs" style={{ color: "#A8967E" }}>
            ...
            {COMPARISONS.filter((c) => c.slug !== cmp.slug).map((c, i) => (
              <span key={c.slug}>
                {i > 0 && " · "}
                <Link href={`/vs/${c.slug}`} className="underline" style={{ color: "#D97706" }}>
