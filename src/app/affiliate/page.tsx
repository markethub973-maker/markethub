...
const inp = { border: "1px solid rgba(245,215,160,0.3)", backgroundColor: "white", color: "var(--color-text)", borderRadius: 8, padding: "8px 12px", fontSize: 14, outline: "none", width: "100%" };
    ...
    <div className="min-h-screen" style={{ backgroundColor: "#FAFAF8" }}>
      ...
      <div className="p-4 max-w-4xl mx-auto space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            { label: "Average commission", value: `${totalCommission > 0 ? (totalCommission / links.length).toFixed(1) : 0}%`, icon: DollarSign, color: "var(--color-primary)" },
                ...
                {filtered.map(link => (
                  <tr key={link.id} style={{ borderBottom: "1px solid rgba(245,215,160,0.1)" }}
                    ...
                    <td className="px-3 py-2.5">
                      <p className="font-semibold" style={{ color: "var(--color-text)" }}>{link.name}</p>
                      {link.category && <span className="text-[10px] px-1.5 py-0.5 rounded-full" style={{ backgroundColor: "rgba(245,158,11,0.1)", color: "var(--color-primary-hover)" }}>{link.category}</span>}
                    </td>
                    ...
                    <td className="px-3 py-2.5">
                      <div className="flex gap-1 justify-end">
                        ...
                        <a href={link.url} target="_blank" rel="noopener noreferrer"
                          className="p-1.5 rounded-lg" style={{ backgroundColor: "rgba(245,158,11,0.08)", color: "var(--color-primary-hover)" }}>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4"
          ...
          <div className="w-full md:max-w-lg rounded-t-2xl md:rounded-2xl overflow-hidden" style={{ backgroundColor: "var(--color-bg-secondary)", maxHeight: "90dvh" }}>
            <div className="flex items-center gap-3 px-4 py-3 shrink-0" style={{ borderBottom: "1px solid rgba(245,215,160,0.3)", backgroundColor: "var(--color-bg)" }}>
              ...
              <p className="font-bold text-sm flex-1" style={{ color: "var(--color-text)" }}>{editId ? "Edit link" : "New affiliate link"}</p>
