...
import { Users, TrendingUp, PlayCircle, Eye, Search, Download, ArrowLeftRight } from "lucide-react";
...
export default function ChannelsPage() {
  ...
  return (
    <div>
      <Header title="Channels" subtitle="Canale YouTube top din trending — date reale" />
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Caută orice canal YouTube (ex: MrBeast, PewDiePie...)"
            value={search}
        ...
        {search.trim().length >= 2 && (
          <div className="rounded-xl overflow-hidden" style={{ backgroundColor: "#FFFCF7", border: "1px solid rgba(245,215,160,0.25)", boxShadow: "0 1px 3px rgba(120,97,78,0.08)" }}>
            <div className="px-5 py-3 flex items-center gap-2" style={{ borderBottom: "1px solid rgba(245,215,160,0.15)" }}>
              ...
              <span className="text-xs font-semibold" style={{ color: "#292524" }}>Rezultate pentru "{search}"</span>
              {ytLoading && <span className="text-xs ml-auto" style={{ color: "#C4AA8A" }}>Se caută...</span>}
            </div>
            <div className="divide-y" style={{ borderColor: "rgba(245,215,160,0.1)" }}>
              {ytResults.length === 0 && !ytLoading ? (
                <p className="px-5 py-4 text-xs" style={{ color: "#C4AA8A" }}>Niciun canal găsit.</p>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex rounded-lg overflow-hidden" style={{ border: "1px solid rgba(245,215,160,0.35)", backgroundColor: "#FFFCF7" }}>
            {REGIONS.map((r) => (
              <button
                ...
                style={region === r.code ? { backgroundColor: "#F59E0B", color: "#1C1814" } : { color: "#78614E" }}
              >
                {r.label}
              </button>
            ))}
          ...
          <select
            ...
            title="Sortează canale"
            aria-label="Sortează canale"
          ...
          <div className="ml-auto flex items-center gap-2">
            {compareList.length > 0 && (
              ...
              >
                ...
                {compareList.length === 2 ? "Compară acum!" : `Selectat ${compareList.length}/2`}
              </button>
            )}
            <button
              type="button"
        ...
        {compareList.length === 1 && (
          <p className="text-xs" style={{ color: "#F59E0B" }}>
            ✓ <b>{compareList[0].name}</b> selectat — alege încă un canal pentru comparație
          </p>
        )}
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs px-1.5 py-0.5 rounded-full" style={{ backgroundColor: "rgba(245,158,11,0.1)", color: "#D97706" }}>Trending {REGIONS.find(r => r.code === region)?.label}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-4 pt-4" style={{ borderTop: "1px solid rgba(245,215,160,0.2)" }}>
                    {[
                      { icon: <Users className="w-3 h-3" />, label: "Abonați", val: formatNumber(ch.subscribers) },
                      { icon: <Eye className="w-3 h-3" />, label: "Total Views", val: formatNumber(ch.totalViews) },
                      { icon: <PlayCircle className="w-3 h-3" />, label: "Videoclipuri", val: formatNumber(ch.videoCount) },
