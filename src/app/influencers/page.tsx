...
const STATUS_COLORS: Record<string, { bg: string; color: string; label: string }> = {
  ...
  contacted: { bg: "rgba(245,158,11,0.1)", color: "var(--color-primary)", label: "Contacted" },
    ...
    <div className="min-h-screen" style={{ backgroundColor: "#FAFAF8" }}>
      ...
      <div className="p-4 max-w-5xl mx-auto space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            { label: "Prospects", value: influencers.filter(i => i.status === "prospect").length, color: "var(--color-primary)" },
          ].map(s => (
              ...
              return (
                <div key={inf.id} className="rounded-xl p-4 space-y-3" style={card}>
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="font-bold text-sm" style={{ color: "var(--color-text)" }}>{inf.name}</p>
                      {inf.niche && <span className="text-[10px] px-2 py-0.5 rounded-full" style={{ backgroundColor: "rgba(245,158,11,0.1)", color: "var(--color-primary-hover)" }}>{inf.niche}</span>}
                    </div>
                  ...
                  <div className="flex gap-3">
                    {inf.ig_username && (
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs font-semibold" style={{ color: "var(--color-text)" }}>{fmt(inf.followers_ig)}</span>
                    ...
                    {inf.tt_username && (
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs font-semibold" style={{ color: "var(--color-text)" }}>{fmt(inf.followers_tt)}</span>
                      </div>
                    )}
                    {inf.price_post > 0 && (
                  ...
                  <div className="flex gap-1 pt-1">
                    ...
                    <button type="button" onClick={() => openEdit(inf)} className="p-1.5 rounded-lg ml-auto" style={{ backgroundColor: "rgba(245,158,11,0.08)", color: "var(--color-primary-hover)" }}><Edit3 className="w-3.5 h-3.5" /></button>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4"
          ...
          <div className="w-full md:max-w-lg rounded-t-2xl md:rounded-2xl overflow-hidden" style={{ backgroundColor: "var(--color-bg-secondary)", maxHeight: "92dvh" }}>
            <div className="flex items-center gap-3 px-4 py-3 shrink-0" style={{ borderBottom: "1px solid rgba(245,215,160,0.3)", backgroundColor: "var(--color-bg)" }}>
              ...
              <p className="font-bold text-sm flex-1" style={{ color: "var(--color-text)" }}>{editId ? "Edit influencer" : "New influencer"}</p>
