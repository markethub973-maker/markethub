...
const PUBLIC_PATHS = [
  ...
  "/api/security/health-check", // Security agent heartbeat — Bearer CRON_SECRET gated
  "/api/cost-monitor",      // Cost & resource monitor — Bearer CRON_SECRET gated
...
export async function proxy(request: NextRequest) {
  ...
  if (host === "brain.markethubpromo.com") {
    ...
    if (
      ...
      pathname.startsWith("/api/brain/dev-pulse") ||          // Claude CLI ↔ Alex sync channel
  ...
  const isCsrfExempt =
    ...
    pathname === "/api/brain/strategy-stack" ||
    ...
    pathname === "/api/brain/alex-loom/avatar-poll" ||
